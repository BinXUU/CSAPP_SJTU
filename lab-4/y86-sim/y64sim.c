...
stat_t nexti(y64sim_t *sim)
{
    ...
    long_t regA_v,regB_v;
    long_t ret,mem_addr,mem_data,rsp_addr;
    ...
    switch (icode)
    {
        ...
        if(get_byte_val(sim->m,next_pc,&regs)==0){
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA=GET_REGA(regs);
    ...
    switch (icode)
    {
        ...
        case I_JMP: /* 7:x imm */
        case I_CALL: /* 8:x imm */
        if (get_long_val(sim->m,next_pc,&imm)==0)
        {
             err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
             return STAT_ADR;
        }
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        mem_addr=get_reg_val(sim->r,regB)+imm;
        regA_v=get_reg_val(sim->r,regA);
        if(!set_long_val(sim->m,mem_addr,regA_v)){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,mem_addr);
            return STAT_ADR;
        }
        sim->pc=next_pc;
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        mem_addr=get_reg_val(sim->r,regA)+imm;
        if (!get_long_val(sim->m,mem_addr,&mem_data))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,mem_addr);
            return STAT_ADR;
        }
        set_reg_val(sim->r,regA,mem_data);
        sim->pc=next_pc;
        break;
        
      case I_ALU: /* 6:x regA:regB */
        regA_v=get_reg_val(sim->r,regA),regB_v=get_reg_val(sim->r,regB);
        ret=compute_alu(ifun,regA_v,regB_v);
        cond=compute_cc(ifun,regA_v,regB_v,ret);
        set_reg_val(sim->r,regB,ret);
      ...
      case I_CALL: /* 8:x imm */
        set_reg_val(sim->r,REG_RSP,next_pc);
        if (imm<0||imm+8>sim->m->len){
            err_print("PC = 0x%lx, Invalid jump instruction address PC=0x%lx",sim->pc,imm);
            return STAT_INS;
        }
        sim->pc=imm;
        break;
      ...
      case I_PUSHQ: /* A:0 regA:F */
        ...
        rsp_addr-=8;
        if(rsp_addr<0||rsp_addr+8>sim->m->len){
            err_print("PC = 0x%lx, Invalid stack address PC=0x%lx",sim->pc,rsp_addr);
            return STAT_INS;
        }
        sim->m[rsp_addr]=regA_v;
        sim->pc=next_pc;
        break;

      case I_POPQ: /* B:0 regA:F */
        regA_v=get_reg_val(sim->r,REG_RSP);
        rsp_addr+=8;
        set_reg_val(sim->r,regA,regA_v);
    	break;
